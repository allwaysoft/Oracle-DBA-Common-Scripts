...
echo "OS_UUID:"`lsattr -El sys0 | grep os_uuid | awk '{print $2}'`   >> $HCLogDirFile0 2>&1
fi
...
IPHost=`hostname`
echo "############BEGIN########"                                     >> $HCLogDirFile0 2>&1
cat /var/hacmp/adm/cluster.log                                       >> $HCLogDirFile0 2>&1
echo "############END########"                                       >> $HCLogDirFile0 2>&1
...
cd $HCWorkDir
tar cvf $MachineSN.$HCHost.$HCLogTime.tar ./$HCHost$HCLogTime
gzip $MachineSN.$HCHost.$HCLogTime.tar

rm -rf ./$HCHost$HCLogTime
echo "Please pick the file $MachineSN.$HCHost.$HCLogTime.tar.gz"
