#!/bin/bash
#Author:CES.HC.RHEL V2.0.O
#Created Date:2015-08-13
#Main Function:Collect Linux formation.
...
then
   ...
   then
     OSType="RHEL 5"
     OSLevel="`cat /etc/redhat-release | awk -F'(' '{print $1}' | awk '{print $NF}'`"
   else
...
UPTIMESTR=`uptime`
DAY=`echo "$UPTIMESTR" | awk -F, '{print $1","$2}'`
HOUR=`echo "$UPTIMESTR" | awk -F, '{print $1}'`

DAYTEST=`echo "$UPTIMESTR" | grep day`
if [ $? = 0 ]
then
  RTSTR=`echo "$DAY" | awk -F "up" '{print $2}'`
else
  RTSTR=`echo "$HOUR" | awk -F"up" '{print $2}'`
fi

UPTIME=`echo ${RTSTR/:/-}`
echo "uptime: $UPTIME"                                               >> $HCLogDirFile0 2>&1
...
> $HCLogDirFile0
echo "########Basic####BEGIN########"                                >> $HCLogDirFile0 2>&1
chkconfig --list 2>/dev/null                                         >> $HCLogDirFile0 2>&1
echo "########Basic####END########"                                  >> $HCLogDirFile0 2>&1
...
> $HCLogDirFile0
echo "########Basic####BEGIN########"                                >> $HCLogDirFile0 2>&1
ps H -A -o cmd,stat,ppid,pid,tid,%cpu,nice,%mem,'vsz=VirtualMem(K)',rss='RealMem(K)',user,time|awk 'BEGIN{OFS=",,";}{if(NF>=12){result="";for ( x = 1; x <= NF-11; x++ ) {result=result" "$x}print result,$(NF-10),$(NF-9),$(NF-8),$(NF-7),$(NF-6),$(NF-5),$(NF-4),$(NF-3),$(NF-2),$(NF-1),$(NF)}}'    >> $HCLogDirFile0 2>&1
echo "########Basic####END########"                                  >> $HCLogDirFile0 2>&1
...
> $HCLogDirFile0
echo "########Basic####BEGIN########"                                >> $HCLogDirFile0 2>&1
tail -n 20000 /var/log/messages                                                >> $HCLogDirFile0 2>&1
echo "########Basic####END########"                                  >> $HCLogDirFile0 2>&1

HCLogDirFile0="$HCLogDir/hclnx.dmesg.log"
> $HCLogDirFile0
echo "########Basic####BEGIN########"                                >> $HCLogDirFile0 2>&1
tail -n 20000 /var/log/dmesg                                                   >> $HCLogDirFile0 2>&1
echo "########Basic####END########"                                  >> $HCLogDirFile0 2>&1

HCLogDirFile0="$HCLogDir/hclnx.secure.log"
> $HCLogDirFile0
echo "########Basic####BEGIN########"                                >> $HCLogDirFile0 2>&1
tail -n 20000 /var/log/secure                                                  >> $HCLogDirFile0 2>&1
echo "########Basic####END########"                                  >> $HCLogDirFile0 2>&1
...
> $HCLogDirFile0
echo "########Basic####BEGIN########"                                >> $HCLogDirFile0 2>&1
tail -n 20000 /var/log/audit/audit.log                                         >> $HCLogDirFile0 2>&1
echo "########Basic####END########"                                  >> $HCLogDirFile0 2>&1
